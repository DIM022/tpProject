...
//TOMAMOS LOS DATOS NECESARIOS
    do{
        ...
        //USAMOS ALGUNAS FUNCIONES DE < FUNCIONES MAIN >
        ...
        if(!validar){
                  //USAMOS ALGUNAS FUNCIONES DE < GESTIONVECTORSOCIO >
                  //FALSE - SI NO HAY COINCIDENCIA
                  //TRUE - SI EXISTE ESE DNI
            validar = verificarExistenciaDniSocio(_dniSocio, vectorSocio);

            //SI NO ENCONTRO EL DNI ENTRA
            if(!validar){
                cout<< "DNI NO ENCONTRADO VERIFICAR SI ES CORRECTO Y/O EXISTENTE"<< endl;
                validar = true;
                //Y COMO ES FALSE SI ENTRA LO PASAMOS A  TRUE PARA QUE VUEK¿LVA A ITERAR
            }
            //SI ES TRUE VUEVE A REPETIR EL LOOP WHILEE
        }
    ...
    do{
        ...
        //TRUE - SI EL INGRESO NO ES CORRECTO
        //FALSE - SI ES CORRECTO
        validar = validarNombre(_nombreLibro);

        //FALSE - ENTRAR Y VERIFICAMOS QUE EXSTA EL LOBRO EN CUENTION
        if(!validar){

            //TRUE SI NO SE ENCONTRO O NO ESTA DISPONIBLE
            //FALSE SI LO ENCONTRO Y ESTA DISPONIBLE
...
bool verificarExistenciaNombreLibro(string _nombreLibro, vector <libro>& vectorLibro){
    ...
    for(size_t i=0; i<vectorLibro.size(); i++){
        ...
        if( _nombreLibro == vectorLibro[i].get_nombre() ){
            ...
            //1 - TRUE ( DISPONIBLIE ) // 0 - FALSE ( NO DISPONIBLE )
